...
impl SqliteStorage {
    async fn get_coverage_evaluation_in_tx(
        ...
        // Derive a lightweight coverage state from the stored rows. Full
        // coverage evaluation (with target-period trust, calendar sync
        // staleness, etc.) is done in takusu-agent; storage provides the
        // conservative baseline used by `get_evaluation_inputs`.
        let state = if !unsettled.is_empty() {
            CoverageState::Stale
        } else if let Some(confirm) = confirmations.first() {
            let now = takusu_types::now_timestamp()
                .map_err(|e| StorageError::Internal(format!("current time unavailable: {e}")))?;
            let start: jiff::Timestamp = confirm.start_at.into();
            let end: jiff::Timestamp = confirm.end_at.into();
            if now >= start && now <= end {
                if confirm.source == "target_period" {
                    CoverageState::Trusted
                } else {
                    CoverageState::TodayCovered
                }
            } else if now > end {
                CoverageState::Stale
            } else {
                CoverageState::Bootstrap
            }
        } else {
            CoverageState::Bootstrap
        };
...
mod tests {
    ...
    async fn coverage_evaluation_round_trip() {
        ...
        assert_eq!(evaluation.state, CoverageState::Stale);
