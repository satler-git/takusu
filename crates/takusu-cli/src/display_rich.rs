//! Rich (`comfy-table`) renderer for the CLI's display commands.
//!
//! All data transformation shared with [`crate::display_simple`] lives in
//! [`crate::display_common`]; this module only owns table rendering and the
...
impl DisplayFormatter for RichFormatter {
    fn display_task_detail(
        ...
        let progress = progress_text(task).unwrap_or_else(|| "—".into());
        let completed = task
            .completed_at
            .as_ref()
    ...
    fn display_tasks(
        ...
        for t in tasks {
            let status_color = status_color(t.status);
            let progress = progress_text(t).unwrap_or_else(|| "—".into());
            let completed = t
                .completed_at
                .as_ref()
                .map(|t| format_datetime(t, tz))
