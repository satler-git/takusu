-- Migration 027: work sessions become top-level entities.
...
--    event was recorded.
CREATE TABLE event_session_match (
    event_id    TEXT,
    session_id  TEXT,
    rn          INTEGER
);

INSERT INTO event_session_match (event_id, session_id, rn)
SELECT
    e.id AS event_id,
    s.id AS session_id,
    ROW_NUMBER() OVER (
...
DROP TABLE event_session_match;
