...
impl std::str::FromStr for InputPath {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            // Short aliases used by clients that send a compact `input_path`.
            "screen" | "screen_capability" => Ok(Self::ScreenCapability),
            "notification" | "notification_capability" => Ok(Self::NotificationCapability),
            "voice" | "explicit_voice_session" => Ok(Self::ExplicitVoiceSession),
            "ambient" | "ambient_wake_word" => Ok(Self::AmbientWakeWord),
            "plain_text" => Ok(Self::PlainText),
            _ => Err("unknown input path"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn input_path_from_str_accepts_compact_and_snake_case_aliases() {
        // Compact aliases used by the mobile app.
        assert_eq!(
            "voice".parse::<InputPath>().unwrap(),
            InputPath::ExplicitVoiceSession
        );
        assert_eq!(
            "screen".parse::<InputPath>().unwrap(),
            InputPath::ScreenCapability
        );
        assert_eq!(
            "notification".parse::<InputPath>().unwrap(),
            InputPath::NotificationCapability
        );
        assert_eq!(
            "ambient".parse::<InputPath>().unwrap(),
            InputPath::AmbientWakeWord
        );
        assert_eq!(
            "plain_text".parse::<InputPath>().unwrap(),
            InputPath::PlainText
        );

        // snake_case wire names generated by OpenAPI clients.
        assert_eq!(
            "explicit_voice_session".parse::<InputPath>().unwrap(),
            InputPath::ExplicitVoiceSession
        );
        assert_eq!(
            "screen_capability".parse::<InputPath>().unwrap(),
            InputPath::ScreenCapability
        );
        assert_eq!(
            "notification_capability".parse::<InputPath>().unwrap(),
            InputPath::NotificationCapability
        );
        assert_eq!(
            "ambient_wake_word".parse::<InputPath>().unwrap(),
            InputPath::AmbientWakeWord
        );

        // Unknown values are rejected instead of silently mapped.
        assert!("unknown".parse::<InputPath>().is_err());
    }
}
