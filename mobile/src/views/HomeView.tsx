  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const firstUnsettled = snapshot?.coverage?.unsettled_intervals?.[0];
        const settlement =
          coverageState === 'stale'
            ? firstUnsettled
              ? `${formatTimeWindow(firstUnsettled.start_at, firstUnsettled.end_at)} の未確定時間を整理してください`
  ...
  const currentTaskCard = useMemo((): TaskCardPresentation | null => {
    ...
    const settlement =
      coverageState === 'stale' && firstUnsettled
        ? {
            question: `${formatTimeWindow(firstUnsettled.start_at, firstUnsettled.end_at)} の未確定時間を整理してください`,
            act: {
              title: '行動',
              actions: [
                {
