...
→ ウェイク判定(既定は AsrTextMatch): 発話区間で streaming ASR の部分認識を
  ウェイクフレーズと照合。軽量な専用 KWS モデルは将来課題
→ 話者照合: ウェイクを通った発話だけを声紋 embedding で判定。常時稼働しない
→ (この時点で) streaming ASR で発話全体を文字起こし
→ (ここで初めて) LLM turn。クラウド課金はここからのみ発生
→ tool execution / proposal
→ TTS or notification
```

既定実装(`WakeWordBackend::AsrTextMatch`)は日本語ウェイクワードの検出に
streaming ASR の部分認識テキストを使う。日本語 KWS の数 MB モデルが
sherpa-onnx に存在しないためで、VAD の後にのみ ASR が走る(常時稼働ではなく
発話区間限定)。軽量な専用 KWS は目標として残り、`SherpaKws` は中国語/英語
キーワード専用のまま評価に使う。

常時稼働するのは VAD のみであり、電池とクラウドコストの問題は VAD の軽さを
ベースにして、AsrTextMatch は発話区間(ウェイク検出まで)だけの追加分となる。
KWS を軽量モデルに戻すこと(日本語対応の既製モデルは現状存在しない)は将来課題。
...
- **wake word の実フレーズと日本語 KWS**: 事前学習モデルは中国語と英語が中心で日本語は未対応。sherpa-onnx の多言語ストリーミング zipformer でも複数文字キーワードは KWS デコーダで発火せず、軽量な日本語 KWS は現状存在しない。既定は streaming ASR の部分認識テキストによるマッチ(`AsrTextMatch`)。軽量な日本語 KWS が現れたら置き換えを検討する。
